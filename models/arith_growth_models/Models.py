...
from models.arith_growth_models.arith_modeling_utils import get_indep_MLE_params, sample_indep_arith, \
  get_dep_MLE_params, sample_dep_RW, get_dep_cov_MLE_params, sample_dep_cov_RW, fit_dep_cov
from modeling_utils.xr_utils import get_dS_dt_timestamps
from modeling_utils.torch_utils import triu_mask_gen_fn, get_dep_SRW_nll_loss, make_Tensor
...
class IndependentModel(Model):
  ...
  def fit_local_params(
    ...
    **kwargs
    ...
    dS_train, dt_train, timestamps_train = get_dS_dt_timestamps(tr_ds.to_dataarray())
    if val_ds is not None:
      dS_val, dt_val, timestamps_val = get_dS_dt_timestamps(val_ds.to_dataarray())
    Sigma = np.diag(vols)

    with torch.no_grad():
      train_hist_info = {
        'train_loss': [get_dep_SRW_nll_loss(make_Tensor(dS_train), make_Tensor(dt_train), make_Tensor(drifts), make_Tensor(Sigma)).detach().item()],
        'val_loss': [get_dep_SRW_nll_loss(make_Tensor(dS_val), make_Tensor(dt_val), make_Tensor(drifts), make_Tensor(Sigma)).detach().item()] if val_ds is not None else [np.nan],
        'train_MAE': [
          mae(
            tr_ds['signal'].to_numpy().T[:, 1:],
            sample_dep_RW(drifts, Sigma, tr_ds['signal'].isel(time=0).to_numpy(), timestamps_train, False)[:, 1:]
          )
        ],
        'val_MAE': [
          mae(
            val_ds['signal'].to_numpy().T[:, 1:],
            sample_dep_RW(drifts, Sigma, val_ds['signal'].isel(time=0).to_numpy(), timestamps_val, False)[:,1:]
          )
  ...
  def simulate(self, market: Market, S_0, dates, num_sims, add_bm: bool = True, **kwargs):
    ...
    for n_sim in range(num_sims):
      for i in range(n_assets):
        sim_res[n_sim, i, :] = sample_indep_arith(
          kwargs['mu'][i],
          kwargs['Sigma'][i],
          S_0[i],
          ts.values[1:],
...
class DependentModel(Model):
  ...
  def fit_local_params(
    ...
    **kwargs
    ...
    return get_dep_MLE_params(tr_ds, val_ds)
...
class DependentCovariateModel(DependentModel, CovariateModel):
  ...
  def fit_global_params(
    ...
    **kwargs
    ...
    """
    (mu, Sigma, theta), train_hist_info = get_dep_cov_MLE_params(
      self.covar_vars, train_dss,
      **kwargs
    )
    self._thetas = theta
    self._mean_covar_correction = 0.
  ...
  def fit_local_params(
    ...
    **kwargs
    ...
    return fit_dep_cov(kwargs.pop('covar_vars', self.covar_vars), tr_ds, theta=kwargs.pop('theta', self._thetas), **kwargs)
  ...
  def simulate(self, market, S_0, dates, num_sims, **kwargs):
    ...
    for n_sim in range(num_sims):
      sim_res[n_sim, :, :] = sample_dep_cov_RW(
