...
from modeling_utils.py_utils import drop_values_from_array
from markets.DerivedVariables import DerivedVariable
from metrics.Metrics import VarType


def zeros_like_nan(arr):
	if isinstance(arr, xr.DataArray) or isinstance(arr, xr.Dataset) or isinstance(arr, xr.Variable):
...
def get_dataset_only_numeric_vars(dataset: xr.Dataset):
	...
	return xr.Dataset.from_dataframe(dataset_df)
...
def append_xr_dataset(ds: xr.Dataset, new_S, dt: int,
                      var_types: Dict[str, VarType], der_var_specs: [DerivedVariable]) -> xr.Dataset:
	"""
	...
	Args:
		...
		var_types: a dict of {str: VarType} mapping all variable names in ds to a VarType
	...
	numeric_vars = [n for n, t in var_types.items() if t in {VarType.QUANTITATIVE, VarType.BINARY}]
	last_frame[numeric_vars] = ds.isel({'time': slice(-5, None)}).mean(dim='time')
...
def get_timestamps(xarr: xr.DataArray):
...
def get_dX_dt_timestamps(xarr: xr.DataArray):
	...
	dX = dX[:, 1:]
	dt = dt[1:]
	return dX, dt, np.cumsum(dt)


def get_dS_dt_timestamps(xarr: xr.DataArray):
	S = xarr.sel(variable='signal').astype(np.float32)
	# dX has shape (n_assets, n_ts) with 1st col = NaN
	dS = (S - S.shift({'time': 1})).to_numpy().T
	# dt has shape (n_ts, ) with 1st entry = NaN
	dt = (xarr.time - xarr.time.shift({'time': 1})).dt.days.values.astype(np.float32)
	...
	dS = dS[:, 1:]
	dt = dt[1:]
	return dS, dt, np.cumsum(dt)


def drop_if_var_na(ds: xr.Dataset, var_name: str, nan_threshold=2/3):
	"""
	Drops assets with NaN frequency more than `na_threshold` `var_name`.

	Args:
		ds: a xr.Dataset with axes ('ID', 'time', 'variables').
		var_name: a variable name.
		nan_threshold: the proportion of NaN values in var_name that are acceptable.

	Returns: a Dataset with potentially fewer entries in the ID axis.
	"""
	nan_rate = ds[var_name].isnull().mean(dim='time')
	nan_indices = [i for i, rate in enumerate(nan_rate.to_numpy()) if rate > nan_threshold]
	return ds.drop_isel(ID=nan_indices), nan_indices
...
def serve_xr_ds(ds: xr.Dataset, batch_size=None, shuffle_mode='none'):
	...
	assert batch_size is None or (
			isinstance(batch_size, int) and batch_size >= 4), '`batch_size` must be None or an integer >= 4.'
	...
	# end `elif shuffle_mode == 'shuffle'`
		...
		while (len(indices) > batch_size) and (len(batch_indices) < batch_size):
			...
			if len(batch_indices) == batch_size:
				...
				indices, indices_st, batch_indices = drop_values_from_array(indices, batch_indices), 0, []
