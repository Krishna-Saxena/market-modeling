...
class DerivedVariable(ABC):
	...
	@abstractmethod
	def init_var(self, xr_ds: xr.Dataset, **kwargs) -> xr.Dataset:
		"""
	...
	@abstractmethod
	def update_var(self, new_xr_ds, **kwargs) -> xr.Dataset:
		"""
...
class EWMATransform(DerivedVariable):
	...
	def init_var(self, xr_ds: xr.Dataset, **kwargs) -> xr.Dataset:
		base_timeseries = xr_ds[self.var_to_avg].to_numpy()
		xr_ds = xr_ds.assign(**{self.op_name: (['time', 'ID'], base_timeseries[0] * np.ones_like(base_timeseries))})
		dt = (xr_ds.time - xr_ds.time.shift({'time': 1})).dt.days.values

		for i in range(1, len(dt)):
			xr_ds[self.op_name].data[i, :] = compute_next_ewma(
				xr_ds[self.op_name].data[i - 1, :],
				base_timeseries[i],
				dt[i],
				self.alpha
			)
	...
	def update_var(self, new_xr_ds, **kwargs) -> xr.Dataset:
		...
		new_xr_ds[self.op_name].data[-1, :] = compute_next_ewma(
			new_xr_ds[self.op_name].data[-2, :],
			new_xr_ds[self.var_to_avg].data[-1, :],
			dt,
			self.alpha
		)
...
class ScaledDeltaTransform(DerivedVariable):
	...
	def init_var(self, xr_ds: xr.Dataset, **kwargs) -> xr.Dataset:
		base_timeseries = xr_ds[self.var_to_transf].to_numpy()
		xr_ds = xr_ds.assign(**{self.op_name: (['time', 'ID'], np.zeros_like(base_timeseries))})
		dt = (xr_ds.time - xr_ds.time.shift({'time': 1})).dt.days.values

		xr_ds[self.op_name].data[1:, :] = (base_timeseries[1:, :] - base_timeseries[:-1, :]) / dt[1:, None]
	...
	def update_var(self, new_xr_ds, **kwargs) -> xr.Dataset:
		...
		"""
		dt = kwargs.get('dt', (new_xr_ds.time - new_xr_ds.time.shift({'time': 1})).dt.days.values[-1])
		new_xr_ds[self.op_name].data[-1, :] = (new_xr_ds[self.var_to_transf].data[-1, :] - new_xr_ds[
			                                                                                   self.var_to_transf].data[-2,
		                                                                                   :]) / dt
		return new_xr_ds
...
class ScaledDeltaLogTransform(DerivedVariable):
	...
	def init_var(self, xr_ds: xr.Dataset, **kwargs) -> xr.Dataset:
		log_base_timeseries = np.log(xr_ds[self.var_to_transf].to_numpy())
		xr_ds = xr_ds.assign(**{self.op_name: (['time', 'ID'], np.zeros_like(log_base_timeseries))})
		dt = (xr_ds.time - xr_ds.time.shift({'time': 1})).dt.days.values

		xr_ds[self.op_name].data[1:, :] = (log_base_timeseries[1:, :] - log_base_timeseries[:-1, :]) / dt[1:, None]
	...
	def update_var(self, new_xr_ds, **kwargs) -> xr.Dataset:
		...
		"""
		dt = kwargs.get('dt', (new_xr_ds.time - new_xr_ds.time.shift({'time': 1})).dt.days.values[-1])
		new_xr_ds[self.op_name].data[-1, :] = (np.log(new_xr_ds[self.var_to_transf].data[-1, :]) - np.log(
			new_xr_ds[self.var_to_transf].data[-2, :])) / dt
		return new_xr_ds
